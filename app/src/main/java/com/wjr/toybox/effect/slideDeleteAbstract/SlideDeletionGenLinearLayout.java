...
public class SlideDeletionGenLinearLayout extends LinearLayout{
    private float downX;
    private float lastX;
    private boolean isScrollHorizontal = false;
    private Scroller scroller;
    public static int SCROLL_TIME = 1000;
    private SlideObserver slideObserver;

    public SlideDeletionGenLinearLayout(Context context) {
        this(context, null, 0);
    }

    public SlideDeletionGenLinearLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SlideDeletionGenLinearLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                //超过最小滑动距离就视为滑动，返回true进行拦截，之后事件都会到onTouchEvent中
    ...
    private View getDeleteView(){
        try {
            BaseHolder holder = (BaseHolder) this.getTag(this.getId());
            return holder.deleteView;
        } catch (Exception e) {
    ...
    private void setIsSlide(boolean isSlide){
        try {
            BaseHolder holder = (BaseHolder) this.getTag(this.getId());
//            holder.dataWrappeds.get(holder.position).setIsSlide(isSlide);
            if (slideObserver != null){
                slideObserver.notifySlideChange(holder.position,isSlide);
            }
        }catch (Exception e){
            e.printStackTrace();
        }
